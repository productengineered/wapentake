...
if(!process.env.ROOM_PLAYWRIGHT_MODULE)throw Error('Set ROOM_PLAYWRIGHT_MODULE to an installed Playwright index.mjs');
const {chromium}=await import(pathToFileURL(process.env.ROOM_PLAYWRIGHT_MODULE));
const root=mkdtempSync(join(tmpdir(),'agent-room-browser-')),repo=join(root,'project'),state=join(root,'state');mkdirSync(repo);
...
try{
  browser=await chromium.launch({headless:true,...(process.env.ROOM_CHROME_EXECUTABLE?{executablePath:process.env.ROOM_CHROME_EXECUTABLE}:{})});
  ...
  const shots=process.env.ROOM_SCREENSHOT_DIR??root;mkdirSync(shots,{recursive:true});await page.screenshot({path:join(shots,'agent-room-desktop.png'),fullPage:true});
  ...
  const download=page.waitForEvent('download');await page.getByRole('button',{name:'Export',exact:true}).click();assert.equal((await download).suggestedFilename(),'agent-room-conversation.md');
  ...
  await page.setViewportSize({width:390,height:844});await page.screenshot({path:join(shots,'agent-room-mobile.png'),fullPage:true});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  assert.deepEqual(errors,[]);console.log(JSON.stringify({status:'passed',fake_consultant_launches:launches,real_model_calls:0,screenshots:shots,checks:['post','literal HTML','invite','cancel','worker start/stop','context preview','source capture','constraint','decision accept','export','reload','desktop/mobile overflow','passive reads do not launch']}));
}catch(error){if(page){console.error(await page.locator('body').innerText());await page.screenshot({path:join(process.env.ROOM_SCREENSHOT_DIR??root,'failure.png'),fullPage:true});}throw error;}
