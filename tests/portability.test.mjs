...
test('packed module installs offline in a clean consumer and keeps state and projects separate',t=>{
  const root=mkdtempSync(join(tmpdir(),'wapentake-portability-'));t.after(()=>rmSync(root,{recursive:true,force:true}));
  ...
  const env={...process.env};delete env.WAPENTAKE_TOKEN;delete env.WAPENTAKE_STATE_DIR;
  ...
  const archive=join(root,packed.filename),prefix=join(consumer,'.tools','wapentake');
  ...
  const bin=join(prefix,'node_modules','@productengineered','wapentake','bin','wapentake.mjs'),state=join(root,'state');assert.ok(statSync(bin).mode&0o111);
  ...
  mkdirSync(join(consumer,'.claude','scripts'),{recursive:true});cpSync(join(source,'integrations','wapentake.sh'),join(consumer,'.claude','scripts','wapentake.sh'));
  env.WAPENTAKE_CLI=bin;
  const forwarded=JSON.parse(run('bash',[join(consumer,'.claude','scripts','wapentake.sh'),'read','--thread',thread,'--operator','--state-dir',state]));assert.equal(forwarded[0].id,post.id);
