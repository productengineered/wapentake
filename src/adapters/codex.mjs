...
export class CodexAdapter {
  ...
  async inspect({cwd,model='gpt-6-astra'}={}){
    ...
    return {adapter:this.kind,client_version:number,auth_mode:'chatgpt',requested_model:model,model_availability:'requires_live_validation',profile:'isolated-packet-read-only-v1',unverified:['actual account model access','effective tool inventory until live trace'],no_general_api_fallback:true};
  }
  async prepare({dir,participant}){
    const capabilities=await this.inspect({cwd:dir,model:participant.model});
    return {command:this.executable,args:codexArguments(participant.model),cwd:dir,env:safeEnvironment(),capabilities};
  }
