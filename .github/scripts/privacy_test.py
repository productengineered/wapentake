...
class PrivacyTests(unittest.TestCase):
    ...
    def test_trusted_scan_ignores_candidate_scanner_and_policy(self):
        ...
        with tempfile.TemporaryDirectory(prefix="wapentake-trusted-policy-") as temporary:
            ...
            binary.parent.mkdir()
            binary.write_text("#!/usr/bin/env python3\nimport pathlib,sys\nconfig=pathlib.Path(sys.argv[sys.argv.index('--config')+1]).read_text()\nif '# trusted policy fixture' in config:\n    print('Trusted policy rejected synthetic fixture',file=sys.stderr)\n    sys.exit(42)\nsys.exit(0)\n")
            binary.chmod(0o755)
            (policy / ".gitleaks.toml").write_text("# trusted policy fixture\n[extend]\nuseDefault = true\n")
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("Trusted policy rejected synthetic fixture", result.stderr)
            Path("credentials.json").write_text("{}")
            git("add", ".")
            git("commit", "-qm", "candidate adds private data")
            result = subprocess.run(["python3", str(scripts / "secrets.py"), "--history", "--trusted-policy"], text=True, capture_output=True)
            self.assertNotEqual(result.returncode, 0)
