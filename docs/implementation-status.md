...
The [archived design](design.md) preserves the initial requirements. This is a usable Mac development preview with its own repository, installer and package archives. The private `productengineered/wapentake` repository now hosts reviewed `main`, with required Mac CI, trusted secret scanning, CodeRabbit completion and review-disposition checks. [Development controls](development.md) describe the protected PR workflow, local hooks and verified draft releases. Publication remains manual. Existing toolkit workflow files, release versions and consumer task stores remain unchanged. Linux and Windows are outside the current scope.
