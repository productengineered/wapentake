...
The three Wapentake gates use commit statuses linked to their workflow logs; their required contexts are bound to the GitHub Actions App. Only the review and repository metadata jobs can write statuses, and only the manually dispatched draft-release job can write releases. Automatic metadata jobs check out the default branch and never execute PR code or consume PR-produced artifacts with their write tokens. A manual dispatch runs the policy at the explicitly selected revision; use `main` for routine refreshes. A maintainer may select a policy-change branch only after its exact commit has passed CI and CodeRabbit review, to verify a new reporting integration before merging it. Ordinary PR tests run with read-only permissions.
